...
class Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.pitch_type == "cwt":
            ...
            f0_phlevel_num = (
                ...
                .float()
                .scatter_add(
                    0, torch.from_numpy(mel2ph).long() - 1, torch.ones(f0.shape)
                )
                .clamp_min(1)
            )
    ...
    def reprocess(self, data, idxs):
        ...
        spker_embeds = (
            np.concatenate(np.array([data[idx]["spker_embed"] for idx in idxs]), axis=0)
            if self.load_spker_embed
            else None
        )
...
class TextDataset(Dataset):
    ...
    def collate_fn(self, data):
        ...
        text_lens = np.array([text.shape[0] for text in texts])
        spker_embeds = (
            np.concatenate(np.array([d[4] for d in data]), axis=0)
            if self.load_spker_embed
            else None
        )

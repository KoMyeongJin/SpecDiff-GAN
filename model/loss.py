...
def get_lsgan_losses_fn():
    ...
    def lsgan_loss_fn(logit_cond, logit_uncond, label_fn,):
        diff_loss = F.mse_loss(logit_cond, label_fn(logit_cond), reduction="mean")
        spec_loss = F.mse_loss(logit_uncond, label_fn(logit_uncond), reduction="mean")
        return 0.5 * (diff_loss + spec_loss)

    def d_loss_fn(r_logit_cond, r_logit_uncond, f_logit_cond, f_logit_uncond):
        r_loss = lsgan_loss_fn(r_logit_cond, r_logit_uncond, torch.ones_like)
        f_loss = lsgan_loss_fn(f_logit_cond, f_logit_uncond, torch.zeros_like)
        return r_loss, f_loss

    def g_loss_fn(f_logit_cond, f_logit_uncond, mask=None):
        f_loss = lsgan_loss_fn(f_logit_cond, f_logit_uncond, torch.ones_like)
        return f_loss
...
class SpecDiffGANLoss(nn.Module):
    """ SpecDiff-GAN Loss """

    def __init__(self, args, preprocess_config, model_config, train_config):
        super(SpecDiffGANLoss, self).__init__()
        ...
        self.lambda_fm = train_config["loss"]["lambda_fm"]
    ...
    def forward(self, model, inputs, predictions, coarse_mels=None, Ds=None):
        ...
        mel_predictions = model.diffusion.denorm_spec(mel_predictions)  # since we use normalized mel
    ...
    def get_fm_loss(self, D_real_diff, D_real_spec, D_fake_diff, D_fake_spec):
        loss_fm = 0
        for j in range(len(D_fake_diff) - 1):
            loss_fm += 0.5 * (F.l1_loss(D_real_diff[j], D_fake_diff[j]))
        for j in range(len(D_fake_spec)):
            loss_fm += 0.5 * (F.l1_loss(D_real_spec[j], D_fake_spec[j]))
        return loss_fm
    ...
    def get_pitch_loss(self, pitch_predictions, pitch_targets):
        ...
        if self.pitch_type == "ph":
            ...
            if self.pitch_type == "cwt":
                cwt_spec = pitch_targets["cwt_spec"]
